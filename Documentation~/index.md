...
The Bugyard Unity SDK lets playtesters file a rich bug report from inside a
...
sends them to your Bugyard backend (`POST /v1/reports`).
...
- The hotkey works under any **Active Input Handling** setting (Player Settings) —
  ...
  backend wired up you can still open the overlay yourself via `Bugyard.Open()`.
...
the backend contract in `bugyard-backend-docs/03-api-contracts.md`. The
